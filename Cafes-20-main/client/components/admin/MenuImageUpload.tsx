...
export function MenuImageUpload({ images, onUploadSuccess, onDeleteImage }: MenuImageUploadProps) {
  ...
  const handleUpload = async (files: FileList) => {
    ...
    try {
      ...
      console.log('🚀 Sending upload request to:', `${API_BASE_URL}/menu-local/upload`);

      const response = await fetch(`${API_BASE_URL}/menu-local/upload`, {
        method: 'POST',
        headers: {
      ...
      setMessage({
        type: 'success',
        text: `Successfully uploaded ${validFiles.length} image${validFiles.length > 1 ? 's' : ''}!`
      });
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/menu-local/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
      ...
      <Card className="bg-[#111111] border-gray-800">
        <CardHeader>
          ...
          <CardDescription className="text-gray-400">
            Upload menu images to display on your website. Supports JPEG, PNG, and WebP formats.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div
